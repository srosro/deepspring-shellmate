...
class GPTAssistantCreator {
    ...
    func getOrUpdateAssistant(assistantBaseName: String, assistantCurrentVersion: String, assistantInstructions: String) async throws -> String {
        ...
        if let existingAssistant = assistants.first(where: { $0.1 == assistantCurrentName }) {
            print("Assistant with name \(assistantCurrentName) already exists with ID: \(existingAssistant.0).")
            return existingAssistant.0
        }

        // Create new assistant
        return try await createNewAssistant(assistantCurrentName: assistantCurrentName, assistantInstructions: assistantInstructions)
    }
    ...
    private func createNewAssistant(assistantCurrentName: String, assistantInstructions: String) async throws -> String {
    ...
    func listAssistants() async throws -> [(String, String)] {
        ...
        guard let response = response as? HTTPURLResponse, response.statusCode == 200,
              let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              let assistants = jsonObject["data"] as? [[String: Any]] else {
            throw NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to list assistants or bad response"])
        }
    ...
    func getOutdatedAssistants(assistants: [(String, String)], assistantBaseName: String, assistantCurrentName: String) -> [String] {
        let regex = try! NSRegularExpression(pattern: "^\(NSRegularExpression.escapedPattern(for: assistantBaseName))")
        var outdatedAssistantIds: [String] = []

        for (id, name) in assistants {
            let range = NSRange(location: 0, length: name.utf16.count)
            if regex.firstMatch(in: name, options: [], range: range) != nil && name != assistantCurrentName {
