//
//  EmptyStateMessages.swift
//  ShellMate
//
//  Created by Daniel Delattre on 01/09/24.
//

import Foundation

struct EmptyStateMessages {
    static let messages: [[String: String]] = [
        [
            "title": "No more Unix-bashing…",
            "subtitle": "untarring a file just got easier.",
            "command": "untar ~/Very-important-files.tar"
        ],
        [
            "title": "How do I watch Star Wars in my terminal?",
            "subtitle": "In a Lay-Z-Boy, with popcorn.",
            "command": "sm \"How do I watch Star Wars in my Terminal?\""
        ],
        [
            "title": "If your AI friend isn’t helpful…",
            "subtitle": "Is it really a friend?",
            "command": "sm \"Help me start a fast.api server with a 'Hello World' app\""
        ],
        [
            "title": "The next great terminal command…",
            "subtitle": "is one typo away."
        ],
        [
            "title": "A wall of text…",
            "subtitle": "is as boring to me as it is to you.",
            "command": "Try highlighting a few words in your Terminal to focus ShellMate’s attention."
        ],
        [
            "title": "Fortune favors the bold…",
            "subtitle": "but it also favors those with a good WiFi connection.",
            "command": "sm \"tell me a fortune\""
        ],
        [
            "title": "Why did the train check its list twice?",
            "subtitle": "It didn’t want to derail any plans!",
            "command": "sl"
        ],
        [
            "title": "Behold! The ancient wisdom of the clipboard…",
            "subtitle": "To retrieve, one must invoke the sacred chant:",
            "command": "sm \"Create a crontab script that keeps a running list of the last five things I have copied to the clipboard.\""
        ],
        [
            "title": "Tired of doing all the talking?",
            "subtitle": "Let me have a turn:",
            "command": "sm \"Say hello world out loud.\""
        ],
        [
            "title": "Dozing off?",
            "subtitle": "Grab a cup of coffee and share the caffeine:",
            "command": "sm \"keep my mac from going to sleep for the next hour\""
        ]
    ]
}
