...
class StartupTerminalWindowHandler {
  static func handleTerminalApp() {
    ...
    if let runningTerminalApp = NSWorkspace.shared.runningApplications.first(where: {
  ...
  private static func isWindowMinimized(_ window: AXUIElement) -> Bool? {
    ...
    if AXUIElementCopyAttributeValue(window, kAXMinimizedAttribute as CFString, &isMinimized)
      == .success
    {
  ...
  private static func unminiaturizeAndActivateWindow(_ window: AXUIElement) {
    ...
    let setResult = AXUIElementSetAttributeValue(
      window, kAXMinimizedAttribute as CFString, unminiaturizeValue)
    if setResult == .success {
  ...
  private static func launchTerminalApp() {
    if let terminalURL = getURLForTerminal() {
      ...
      NSWorkspace.shared.openApplication(
        at: terminalURL, configuration: configuration, completionHandler: nil)
    } else {
