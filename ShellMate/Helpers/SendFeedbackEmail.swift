...
class UserSystemProperties {
    ...
    class func getSerialNumber() -> String? {
        ...
        if let key = kIOPlatformSerialNumberKey as String? {
            serialNumber = IORegistryEntryCreateCFProperty(platformExpert, key as CFString, kCFAllocatorDefault, 0).takeUnretainedValue() as? String
        }
    ...
    class func getIOPlatformUUID() -> String {
        ...
        if let key = kIOPlatformUUIDKey as String? {
            uuid = IORegistryEntryCreateCFProperty(platformExpert, key as CFString, kCFAllocatorDefault, 0).takeUnretainedValue() as? String
        }
        IOObjectRelease(platformExpert)
        return uuid ?? "Unknown"
    }
