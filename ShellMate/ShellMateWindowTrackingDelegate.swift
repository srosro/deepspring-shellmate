...
class GhostWindow: NSWindow {
    ...
    static func getInstance(appWindowPosition: NSRect) -> GhostWindow {
        if sharedInstance == nil {
            let ghostRect = NSRect(x: appWindowPosition.origin.x, y: appWindowPosition.origin.y, width: 100, height: 100) // Placeholder rect; will be updated by notification
    ...
    init(contentRect: NSRect) {
        ...
        self.isOpaque = false
        self.backgroundColor = NSColor(calibratedRed: 8/255, green: 133/255, blue: 245/255, alpha: 0.8)
        self.level = .floating
    ...
    private func setupNotificationObservers() {
        NotificationCenter.default.addObserver(self, selector: #selector(handleGhostWindowStateChange(_:)), name: .ghostWindowStateDidChange, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(handleMousePositionCloseToTerminalBorder(_:)), name: .mousePositionCloseToTerminalBorder, object: nil)
    }

    deinit {
        NotificationCenter.default.removeObserver(self, name: .ghostWindowStateDidChange, object: nil)
        NotificationCenter.default.removeObserver(self, name: .mousePositionCloseToTerminalBorder, object: nil)
    }
        ...
        case "update", "show":
            if let appWindowPosition = userInfo["appWindowPosition"] as? NSRect,
               let terminalPosition = userInfo["terminalPosition"] as? (position: CGPoint, size: CGSize) {
                let relativePosition = checkWindowPositionRelativeToTerminal(appWindowPosition: appWindowPosition, terminalWindowPosition: terminalPosition.position, terminalWindowSize: terminalPosition.size)

                if relativePosition == "float" {
                    self.orderOut(nil)
                } else {
                    let frame = calculateGhostWindowFrame(appWindowPosition: appWindowPosition, terminalPosition: terminalPosition)
    ...
    private func calculateGhostWindowFrame(appWindowPosition: NSRect, terminalPosition: (position: CGPoint, size: CGSize)) -> NSRect {
        appRelativePositionToTerminalWindow = checkWindowPositionRelativeToTerminal(appWindowPosition: appWindowPosition, terminalWindowPosition: terminalPosition.position, terminalWindowSize: terminalPosition.size)
        ...
        if appRelativePositionToTerminalWindow == "left" {
            ghostRect = NSRect(x: terminalPosition.position.x, y: terminalPosition.position.y, width: borderWidth, height: terminalPosition.size.height)
        } else if appRelativePositionToTerminalWindow == "right" {
            ghostRect = NSRect(x: terminalPosition.position.x + terminalPosition.size.width - borderWidth, y: terminalPosition.position.y, width: borderWidth, height: terminalPosition.size.height)
        } else {
            ghostRect = NSRect(x: appWindowPosition.origin.x - borderWidth, y: appWindowPosition.origin.y, width: borderWidth, height: appWindowPosition.size.height)
        }
    ...
    @objc private func handleMousePositionCloseToTerminalBorder(_ notification: Notification) {
        ...
        if isMouseCloseToTerminalBorder && (terminalBorderPosition == appRelativePositionToTerminalWindow) {
            ...
            if mouseCloseToBorderFeedbackWindow == nil {
                let feedbackRect = NSRect(x: feedbackXPosition, y: self.frame.minY, width: feedbackWindowWidth, height: self.frame.height)
                mouseCloseToBorderFeedbackWindow = NSWindow(contentRect: feedbackRect, styleMask: .borderless, backing: .buffered, defer: false)
                mouseCloseToBorderFeedbackWindow?.isOpaque = false
                mouseCloseToBorderFeedbackWindow?.backgroundColor = NSColor(calibratedRed: 8/255, green: 133/255, blue: 245/255, alpha: 0.2)
        ...
        print("Mouse is close to terminal border: \(isMouseCloseToTerminalBorder), at border: \(terminalBorderPosition ?? "nil")")
    }
...
class ShellMateWindowTrackingDelegate: NSObject {
    ...
    private func handleMouseUp(_ event: NSEvent) {
        ...
        if let terminalWindowPosition = getTerminalWindowPositionAndSize() {
            updateGhostWindowPosition(appWindowPosition: appWindowPosition, terminalWindowPosition: terminalWindowPosition)
            handleRelativePositionUpdate(appWindowPosition: appWindowPosition, terminalWindowPosition: terminalWindowPosition)
        }
    ...
    private func getTerminalWindowPositionAndSize() -> (position: CGPoint, size: CGSize)? {
        if let positionDelegate = windowPositionDelegate, let result = positionDelegate.getTerminalWindowPositionAndSize() {
    ...
    private func handleRelativePositionUpdate(appWindowPosition: NSRect?, terminalWindowPosition: (position: CGPoint, size: CGSize)) {
        let relativePosition = checkWindowPositionRelativeToTerminal(appWindowPosition: appWindowPosition, terminalWindowPosition: terminalWindowPosition.position, terminalWindowSize: terminalWindowPosition.size)
    ...
    private func handleAppWindowPositionChange(notification: CFString) {
        ...
        if notification == kAXResizedNotification as CFString {
            ...
            if !isDraggingWindow {
                ...
                if let terminalWindowPosition = getTerminalWindowPositionAndSize() {
                    createAndShowGhostWindow(appWindowPosition: appWindowPosition, terminalWindowPosition: terminalWindowPosition)
                }
                mousePositionTrackingManager.startMonitoringMousePositionEvents()
            } else {
                // Update ghost window position during drag
                if let terminalWindowPosition = getTerminalWindowPositionAndSize() {
                    postGhostWindowStateNotification(action: "update", appWindowPosition: appWindowPosition, terminalWindowPosition: terminalWindowPosition)
                }
            }
        }
    }

    private func printTerminalWindowPosition(_ terminalWindowPosition: (position: CGPoint, size: CGSize)) {
        print("Terminal window position: \(terminalWindowPosition.position), Size: \(terminalWindowPosition.size)")
    }

    private func hideGhostWindow() {
        NotificationCenter.default.post(name: .ghostWindowStateDidChange, object: nil, userInfo: ["action": "hide"])
        ghostWindowController = nil
    }
    
    private func updateGhostWindowPosition(appWindowPosition: NSRect?, terminalWindowPosition: (position: CGPoint, size: CGSize)) {
        guard let appWindowPosition = appWindowPosition else { return }
        ...
        NotificationCenter.default.post(name: .ghostWindowStateDidChange, object: nil, userInfo: userInfo)
    }

    private func postGhostWindowStateNotification(action: String, appWindowPosition: NSRect?, terminalWindowPosition: (position: CGPoint, size: CGSize)) {
        guard let appWindowPosition = appWindowPosition else { return }
    ...
    private func createAndShowGhostWindow(appWindowPosition: NSRect?, terminalWindowPosition: (position: CGPoint, size: CGSize)) {
        guard let appWindowPosition = appWindowPosition else { return }
...
class MousePositionTrackingManager {
    ...
    func startMonitoringMousePositionEvents() {
        localMousePositionEventMonitor = NSEvent.addLocalMonitorForEvents(matching: [.mouseMoved, .leftMouseDragged]) { [weak self] event in
    ...
    private func handleMousePositionEvent(_ event: NSEvent) {
        ...
        guard currentTime.timeIntervalSince(lastMousePositionEventTime) > mousePositionDebounceInterval else {
            return
        }
        ...
        if let terminalWindowPosition = getTerminalWindowPositionAndSize() {
            ...
            let distanceToRightBorder = abs(mouseLocation.x - (terminalWindowPosition.position.x + terminalWindowPosition.size.width))
            ...
            NotificationCenter.default.post(name: .mousePositionCloseToTerminalBorder, object: nil, userInfo: ["isCloseToTerminalBorder": isClose, "terminalBorder": border ?? ""])
        }
    }
...
func checkWindowPositionRelativeToTerminal(appWindowPosition: NSRect?, terminalWindowPosition: CGPoint, terminalWindowSize: CGSize) -> String {
    ...
    if appWindowCenterX < terminalWindowLeftX - halfAppWidth || appWindowCenterX > terminalWindowRightX + halfAppWidth {
        return "float"
    }
